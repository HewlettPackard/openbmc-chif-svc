...
struct pkt_812b {
	uint32_t ErrorCode; // EV Error/Return Code
	uint8_t name[EV_NAME_MAX_LEN];   // EV Name
...
void init_smif(void) {
    ...
    if (gethostname(gHostName, IODNETCFG_HOST_NAME_SIZE)) {
        printf("hostname failed: errno: %d : %s\n", errno,  strerror(errno));
    } else {
        dbPrintf("hostname: %s\n", gHostName);
    }
    ...
    if (getdomainname(gDomainName, IODNETCFG_DOMAIN_NAME_SIZE)) {
        printf("domainname failed: errno: %d : %s\n", errno,  strerror(errno));
    } else {
    ...
    if(recvMsg->address == 0xffff && recvMsg->segment == 0xff) {
        ...
        if (bus == -1) {
            ...
            if (retry > MAX_I2C_RETRIES) {
                ...
                printf("Unable to access I2C device at bus: %d addr: %02x\n", bus, (uint8_t)(recvMsg->address >> 1));
                respMsg->ErrorCode = I2C_SEGMENT_DOES_NOT_EXIST;
            }
...
int SmifPkt_012c(void *recv, void *resp)
{
	...
	switch(recvMsg->flags) {
		...
		case 0x02: //EV delete
			...
			if(recvMsg->name[EV_NAME_MAX_LEN-1] != 0x00) {
				respMsg->ErrorCode = 0x03; //EV name too large. EV's are supposed to be null terminated.
...
int SmifPkt_0130(void *recv, void *resp)
{
	...
	if(ev_size<0) {
		...
		memcpy(respMsg->name, recvMsg->name, sizeof(respMsg->name));
	}
...
int SmifPkt_0146(void *recv, void *resp)
{
    ...
    memcpy(&evt.data, &recvMsg->buf, recvMsg->evtVarLen < sizeof(evt.data) ? recvMsg->evtVarLen : sizeof(evt.data));
    int rv =0;
    switch (recvMsg->evtType) {
    case EVT_IML:
            rv = decode_text(LOGS_IMLDATA, &evt, event_message, (unsigned short)sizeof(buffer), TEXT_DESC);
            dbPrintf("smif_0146 %d: quick event add to IML\n%s\n", rv, event_message);
            break;
        case EVT_IEL:
            rv = decode_text(LOGS_GELDATA, &evt, event_message, (unsigned short)sizeof(buffer), TEXT_DESC);
            dbPrintf("smif_0146 %d: quick event add to IEL\n%s\n", rv, event_message);
            break;
        case EVT_SL:
            rv = decode_text(LOGS_SLDATA, &evt, event_message, (unsigned short)sizeof(buffer), TEXT_DESC);
            dbPrintf("smif_0146 %d: quick event add to SL\n%s\n", rv, event_message);
            break;
        default:
